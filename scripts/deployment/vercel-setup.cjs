 ...
 * vercel-setup.cjs
 * 
 * This script will be run before the build process to ensure all files
 * are properly prepared for the Vercel build environment.
 * 
 * Run this with: node vercel-setup.cjs
 */

const fs = require('fs');
const path = require('path');

// Path to the notification service file
const notificationServicePath = path.join(__dirname, 'src', 'services', 'notificationService.js');

// The modified content that doesn't rely on authService imports
...
try {
  ...
  if (fs.existsSync(notificationServicePath)) {
    ...
    fs.writeFileSync(notificationServicePath, modifiedContent);
    console.log('✅ Successfully updated notificationService.js');
  } else {
    console.error('❌ Could not find notificationService.js at', notificationServicePath);
  }
} catch (error) {
  console.error('❌ Error updating notificationService.js:', error);
}
